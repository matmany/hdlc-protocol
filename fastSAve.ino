...
void loop()
{
  ...
  //CHAVES ATIVA SEC1
    ...
    for (int i = 0; i < addressSize; i++) {
      frame[(flagSize + i)] = addressSec1[i];
    }
    for (int i = 0; i < dataSize; i++) {
      frame[(flagSize + addressSize + i)] = data1[i];
    }
    for (int i = 0; i < flagSize; i++) {
      frame[(flagSize + addressSize + dataSize + i)] = flag1[i];
    }
    sendDataToS1();
  }
  ...
  //CHAVES ATIVA SEC2
    ...
    for (int i = 0; i < addressSize; i++) {
      frame[(flagSize + i)] = addressSec2[i];
    }
    for (int i = 0; i < dataSize; i++) {
      frame[(flagSize + addressSize + i)] = data2[i];
    }
    for (int i = 0; i < flagSize; i++) {
      frame[(flagSize + addressSize + dataSize + i)] = flag2[i];
    }
    sendDataToS2();
  }
